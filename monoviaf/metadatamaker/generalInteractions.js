(function (global) {
  ...
  function buildMonoviafAuthorMarkup(index) {
    let markup = '';
    markup += '<label for="family_name' + index + '" class="insert insert_family_name additional_insert" onClick=\'insertMenu("family_name' + index + '");\'>Insert other Characters</label><br>';
    markup += '<div id="insert-family_name' + index + '" class="additional_menu"></div>';
    markup += '<div class="added-author flex gap-2 items-center mb-2"><input type="text" class="author translit-listen input-base flex-grow" id="family_name' + index + '" placeholder="Family Name, Given Name"> ';
    markup += '<select name="role' + index + '" id="role' + index + '" class="input-base w-auto"><option value="art">artist</option><option selected value="aut">author</option><option value="ctb">contributor</option><option value="edt">editor</option><option value="ill">illustrator</option><option value="trl">translator</option></select></div>';
    ...
    markup += '<label for="translit_family_name' + index + '" class="insert insert_family_name hidden translit translit-family_name' + index + '" onClick=\'insertMenu("translit_family_name' + index + '");\'>Insert other Characters</label><br>';
