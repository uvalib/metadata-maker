	...
	if (form) {
		form.addEventListener("submit", function (event) {
			...
			for (var i = 0; i < loopCounter; i++) {
				...
				if (fastIdElem && keywordElem && checkExists(fastIdElem.value) && checkExists(keywordElem.value)) {
					if (keywordElem.value.substring(keywordElem.value.length - 1) == ']') {
						...
						fast_array.push([keywordElem.value.substring(0, endpoint - 2), fastIdElem.value, fastTypeElem ? fastTypeElem.value : '', fastIndElem ? fastIndElem.value : '']);
					}
					else {
			...
			var recordObject = {
				...
				sudoc: getValue("sudoc"),
				report_number: getValue("report-number"),
